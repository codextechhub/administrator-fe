...
import { ArrowLeft, Info, Mail, ShieldOff } from "lucide-react";
...
/**
 * "Add Staff & Invitations" - the real screen, not an explanation of one.
 *
 * This used to say the surface opened at go-live, which was true: the only
 * endpoint that could create a school user was the platform's, gated on
 * `platform.team.*`, which a school administrator does not hold. A school-scoped
 * `/v1/i/me/staff/` now exists on the pending-tenant surface, so the step a
 * school is asked for is a step a school can actually do.
 *
 * The design draws this as one of two tabs beside "Roles & Permissions". The
 * tab strip arrives with that tab rather than before it: a strip with one live
 * tab and one empty one is worse than no strip.
 *
 * Skipping and marking done stay on the control-room card, which is where the
 * design puts them and where the reader already is.
 */
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="min-w-0 max-w-[60ch]">
          ...
          <p className="mt-1 text-sm text-gray-01 text-pretty">
            Invite the people who will operate the system. They each get an
            email with a link that sets their own password, so nobody shares an
            account.
          </p>
        </div>
        <Button
